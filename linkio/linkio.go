...
func NewLinkReader(r io.Reader, link *Link) (s *LinkReader) {
...
func NewLink(kbps int) (l *Link) {
	l = &Link{ in: make(chan LinkRequest) }
	_ = l.SetSpeed(kbps)
...
func (l *LinkReader) Read(buf []byte) (n int, err os.Error) {
  // read small chunks at a time, even if they ask for more -- lets us
	// demo the slow speed better
	toRead := min(len(buf), 100)
